...
class ChatWriterThread(QThread):
    ...
    def run(self):            
        with self.get_chromedriver(use_proxy=True, user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36') as self.driver:
            print(f'Окно браузера запущено. [{self.account_name}]')
            
            #site = f'https://yandex.ru/internet'
            ...
            try:
                ...
                button2.click()
                print('Кнопки успешно нажата.')
            except NoSuchElementException:
                print('Кнопки 1 не найдено.')
            
            try:
                element = self.driver.find_element(By.XPATH, '//*[@id="headlessui-menu-button-3"]/div/img')
                if element:
                    print('Элемент найден')
            except NoSuchElementException:
                print('Элемента с именем канала не найдено.')
    ...
    def get_chromedriver(self, use_proxy=False, user_agent=None):
        ...
        user_data_dir = '/headles-bot/user_data'
